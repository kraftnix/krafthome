...
{
  imports = [
    (lib.mkAliasOptionModule [ "khome" "shell" "atuin" "settings" ] [ "programs" "atuin" "settings" ])
  ];
  ...
  config = mkIf cfg.enable {
    programs.atuin = {
      enable = true;
      daemon.enable = mkDefault cfg.enableSystemdDaemon;
      daemon.logLevel = mkDefault "info";
      settings = {
