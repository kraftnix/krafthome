...
with lib; let
  ...
  in
    genReadableColors' (loadyaml {inherit src;});
  ...
  hexStrOnly = attrSet:
    filterAttrs
    (n: v:
      (builtins.typeOf v == "string") && (builtins.stringLength v == 6))
    attrSet;
  # Superset of theme colors with helpers + overrides
  genColors' = theme: overrides:
    with theme;
      (hexStrOnly theme)
      // {
        # Useful Helpers
