{
  ...
  allVimPlugins = nixpkgs: sources: (import ./vim-plugin.nix nixpkgs sources [
    # "nvim-nu"
    "nvim-telescope-hop"
    "nvim-guess-indent"
    "neozoom-nvim"
  ...
  importNuPlugin = prev: sources: name: cargoHash:
    prev.callPackage (
      import
      ./nu_plugins/${name}.nix
      sources.${name}
  ...
  renamePlugins = lib.mapAttrs' (
    name: plugin:
      lib.nameValuePair (lib.replaceStrings ["nu_plugin_"] [""] name) plugin
  );
